...
若 `PAPER_LIBRARY_DIR` 为空，必须先询问用户论文库绝对路径，并说明该目录会存放每篇论文的独立文件夹、英文/中文 PDF、源码工作区 `.tmp_arxiv` 和 `download.env` 元数据。在用户答复前不得假定或使用当前目录，也不得继续下载或翻译。用户答复后，将所选绝对路径写回本机已安装的 skill 文件（即当前 `SKILL.md`）中的 `PAPER_LIBRARY_DIR="..."`；仓库副本仍保持 `PAPER_LIBRARY_DIR=""`，不要写入具体个人路径。仅当用户明确拒绝提供长期目录时，才可把当前工作目录作为本次临时 `PAPER_LIBRARY_DIR` 使用，且不得写回配置。
