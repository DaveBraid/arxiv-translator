...
若 `PAPER_LIBRARY_DIR` 为空，先询问用户论文库绝对路径，并说明该目录会存放每篇论文的独立文件夹、最终中文 PDF、源码工作区 `.tmp_arxiv`、`download.env` 元数据和可选源码备份。若用户不提供，则使用当前目录作为本次 `PAPER_LIBRARY_DIR`。
...
└── 2501.12948v2 - Paper Title/
    ├── 2501.12948v2 - Paper Title.pdf
    ├── download.env
    ├── .tmp_arxiv/
    │   └── 2501.12948v2/
    │       ├── <arXiv source files>
    │       └── <translated .tex files>
    └── translated-sources/        # 可选；清理前备份 .tex/.bbl 时生成
        └── 2501.12948v2/
            ├── *.tex
            └── *.bbl
```
...
- **标题要求：** `\title{}` 须改为自然中文题名，不保留英文原题或中英并列；输出 PDF 文件名使用第二步的 `PDF_PATH`，即与单篇文献目录同名。
...
编译成功后默认保留 `$PAPER_DIR/.tmp_arxiv`，方便用户检查 PDF 后继续微调翻译源码。只有用户明确要求清理、或你已经完成必要检查且确认不再需要源码时，才调用：
...
```bash
python3 {SKILL_DIR}/scripts/cleanup.py "$PAPER_DIR" "$PAPER_DIR/translated-sources"
```
